...
export class CubismPhysics {
   ...
   */
  public release(): void {
    this._physicsRig = void 0 as any;
    this._physicsRig = null as any;
  }
   ...
   * @param size Buffer size
   */
  public parse(json: PhysicsJson): void {
    ...
    this._physicsRig.inputs.updateSize(
      json.Meta.TotalInputCount,
      CubismPhysicsInput,
      true
    );
    this._physicsRig.outputs.updateSize(
      json.Meta.TotalOutputCount,
      CubismPhysicsOutput,
      true
    );
    this._physicsRig.particles.updateSize(
      json.Meta.VertexCount,
      CubismPhysicsParticle,
      true
    );
    ...
    for (let i = 0; i < this._physicsRig.settings.getSize(); ++i) {
      this._physicsRig.settings.at(
        i
      ).normalizationPosition.minimum = json.PhysicsSettings[i].Normalization.Position.Minimum
      this._physicsRig.settings.at(
        i
      ).normalizationPosition.maximum = json.PhysicsSettings[i].Normalization.Position.Maximum
      this._physicsRig.settings.at(
        i
      ).normalizationPosition.defalut = json.PhysicsSettings[i].Normalization.Position.Default

      this._physicsRig.settings.at(
        i
      ).normalizationAngle.minimum = json.PhysicsSettings[i].Normalization.Angle.Minimum
      this._physicsRig.settings.at(
        i
      ).normalizationAngle.maximum = json.PhysicsSettings[i].Normalization.Angle.Maximum
      this._physicsRig.settings.at(
        i
      ).normalizationAngle.defalut = json.PhysicsSettings[i].Normalization.Angle.Default
      ...
      for (let j = 0; j < this._physicsRig.settings.at(i).inputCount; ++j) {
        ...
        this._physicsRig.inputs.at(
          inputIndex + j
        ).reflect = json.PhysicsSettings[i].Input[j].Reflect
        ...
        this._physicsRig.inputs.at(
          inputIndex + j
        ).source.id = json.PhysicsSettings[i].Input[j].Source.Id;
      }
      ...
      for (let j = 0; j < this._physicsRig.settings.at(i).outputCount; ++j) {
        ...
        this._physicsRig.outputs.at(
          outputIndex + j
        ).vertexIndex = json.PhysicsSettings[i].Output[j].VertexIndex
        this._physicsRig.outputs.at(
          outputIndex + j
        ).angleScale = json.PhysicsSettings[i].Output[j].Scale
        this._physicsRig.outputs.at(
          outputIndex + j
        ).weight = json.PhysicsSettings[i].Output[j].Weight
        this._physicsRig.outputs.at(outputIndex + j).destination.targetType =
          CubismPhysicsTargetType.CubismPhysicsTargetType_Parameter;

        this._physicsRig.outputs.at(
          outputIndex + j
        ).destination.id = json.PhysicsSettings[i].Output[j].Destination.Id
        ...
        this._physicsRig.outputs.at(
          outputIndex + j
        ).reflect = json.PhysicsSettings[i].Output[j].Reflect
      }
      ...
      for (let j = 0; j < this._physicsRig.settings.at(i).particleCount; ++j) {
        this._physicsRig.particles.at(
          particleIndex + j
        ).mobility = json.PhysicsSettings[i].Vertices[j].Mobility
        this._physicsRig.particles.at(
          particleIndex + j
        ).delay = json.PhysicsSettings[i].Vertices[j].Delay
        this._physicsRig.particles.at(
          particleIndex + j
        ).acceleration = json.PhysicsSettings[i].Vertices[j].Acceleration
        this._physicsRig.particles.at(
          particleIndex + j
        ).radius = json.PhysicsSettings[i].Vertices[j].Radius
        this._physicsRig.particles.at(
          particleIndex + j
        ).position = new CubismVector2(json.PhysicsSettings[i].Vertices[j].Position.X, json.PhysicsSettings[i].Vertices[j].Position.Y)
      }
